    ...
    for (let source in r.data) {
        ...
        if (r.data[source] && r.data[source].edges && r.data[source].edges.length) {
            r.data[source].edges = r.data[source].edges.filter(({ node }) => !exclude.some((excludedRoute) => {
                // const slug = (source === `allMarkdownRemark` || source === `allMdx`) && node.fields && node.fields.slug ? node.fields.slug.replace(/^\/|\/$/, ``) : node.slug.replace(/^\/|\/$/, ``)
        ...
        if (mapping[type] && mapping[type].sitemap) {
            ...
            if (currentSource) {
                ...
                currentSource.edges.map(({ node }) => {
                    ...
                    console.log(`mapping[type].path`, mapping[type].path)
                    ...
                    if (mapping[type].path) {
                        console.log(`node.path 1`, node.path)
                        console.log(`node.slug`, node.slug)
                        node.path = path.join(mapping[type].path, node.slug)
                        console.log(`node.path 2`, node.path)
                    } else {
                        node.path = node.slug
                    }

                    console.log(`node.path 3`, node.path)

                    // get the real path for the node, which is generated by Gatsby
                    node = getNodePath(node, allSitePage)

                    console.log(`url.resolve`, url.resolve(siteUrl, node.path))
...
exports.onPostBuild = async ({ graphql, pathPrefix }, pluginOptions) => {
    ...
    // We always query siteAllPage as well as the site query to FIXME: siteAllPage = allSitePage
    ...
    // Instanciate the Ghost Sitemaps Manager FIXME: Instanciate = Instantiate
